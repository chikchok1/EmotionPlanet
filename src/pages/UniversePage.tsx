...
export function UniversePage({ navigate }: UniversePageProps) {
  const { state, getDominantEmotion, getPlanetRecords, getTodayRecord, setViewingPlanet } = useEmotionPlanet();
  ...
  const handlePlanetClick = (planet: Planet, index: number) => {
    ...
    const emotion = !isLocked && todayEmotion
      ? todayEmotion
      : completed?.dominantEmotion ?? getLatestEmotion(planetRecords, getDominantEmotion(planetRecords));
